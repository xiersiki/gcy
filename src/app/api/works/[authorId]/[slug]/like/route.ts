...
export async function POST(
  ...
  const limited = ensureRateLimit(req, requestId, `like:${workId}`, 10, 10_000)
  ...
  return jsonOk({ liked: true }, { headers: withRequestIdHeaders(requestId) })
}
...
export async function DELETE(
  ...
  const limited = ensureRateLimit(req, requestId, `unlike:${workId}`, 10, 10_000)
  if (limited) return limited
  ...
  return jsonOk({ liked: false }, { headers: withRequestIdHeaders(requestId) })
}
